...
determine_the_aparent_magnitude_Form,

determine_the_absolute_magnitude_Form,

delete_objects_used_in_calculations_Form,

determine_distance_from_Earth_Form,

ratio_in_brightness_between_two_stars_Form,

light_years_in_parsecs_Form,

calculate_the_annual_paralax_Form,

SumofAttributesofnthDayForm,

Determine_the_visibility_of_a_star_in_seasons_Form,

calculate_the_flux_density_Form,

stars_visible_in_equinox_dates_Form,

stars_visible_in_solstice_dates_Form,

display_stars_most_visible_in_equinox_dates_Form,

display_stars_most_visible_in_solstice_dates_Form,

delete_the_DaysInSearchEngine_using_ids_Form, 

set_total_lunar_eclipses_Form,

lunar_eclipses_stars_most_visible_Form,

determine_the_relative_position_of_stars_Form,)
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			obj.first_date_with_total_lunar_eclipse.year ==  
				...
				# td = (timedelta(days=res[0]*365 + res_2[0]+ (res_1[0]*4*7)) )
				# +timedelta(days=obj.second_date_with_total_lunar_eclipse.day-
				# 		obj.first_date_with_total_lunar_eclipse.day )
			...
			# 	td = (td - timedelta(days=abs(int(obj.first_date_with_total_lunar_eclipse.day)-
			# 		int(obj.second_date_with_total_lunar_eclipse.day)))+timedelta(days=2))
			...
			# 	td = abs(td + timedelta(days=30+(obj.first_date_with_total_lunar_eclipse.day-
			# 			obj.second_date_with_total_lunar_eclipse.day)))
...
def determine_the_aparent_magnitude(request):


	if request.method == 'POST':

		form = determine_the_aparent_magnitude_Form(request.POST)
		...
		if form.is_valid():
			...
			obj.aparent_magnitude_calculation = obj.absolute_magnitude_calculation - (5 + 5*math.log10(1/obj.distance_from_Earth_calculation))

			obj.used_in_calculations = True

			print(obj.aparent_magnitude_calculation)
			
			obj.save()
			
			form = determine_the_aparent_magnitude_Form()
	...
	template_path = "Days/determine-the-aparent-magnitude.html"
...
def determine_the_absolute_magnitude(request):


	if request.method == 'POST':

		form = determine_the_absolute_magnitude_Form(request.POST)
		...
		if form.is_valid():
			...
			obj.absolute_magnitude_calculation = obj.aparent_magnitude_calculation + 5 + 5*math.log10(1/obj.distance_from_Earth_calculation)

			obj.used_in_calculations = True

			print(obj.absolute_magnitude_calculation)
			
			obj.save()
			
			form = determine_the_absolute_magnitude_Form()
	...
	template_path = "Days/determine-the-absolute-magnitude.html"
...
def delete_objs_used_in_calculations(request):
		...
		if request.method == 'POST':
			...
			form = delete_objects_used_in_calculations_Form(request.POST)

			if form.is_valid():

				form.save()

				qs = Stars.objects.all()

				for i in qs:
					if i.used_in_calculations == True:
						i.delete()

			form = delete_objects_used_in_calculations_Form()
		...
		template_path = "Days/delete-objs-used-in-calculations.html"
...
def determine_distance_from_Earth(request):

	form = determine_distance_from_Earth_Form(request.POST)
	...
	if form.is_valid():
		...
		obj.distance_from_Earth_calculation =  math.pow(10, (obj.aparent_magnitude_calculation + 5 - obj.absolute_magnitude_calculation) / 5)

		obj.used_in_calculations = True

		print(obj.distance_from_Earth_calculation)
		
		obj.save()
		
		form = determine_distance_from_Earth_Form()
	...
	template_path = "Days/determine-distance-from-Earth.html"
...
def determine_the_ratio_in_brightness_between_visible_stars(request):


	form = ratio_in_brightness_between_two_stars_Form(request.POST)
	...
	if form.is_valid():
		...
		obj.ratio_in_brightness_res = math.pow(10, 0.4 *(obj.first_aparent_magnitude_ratio_brightness - obj.second_aparent_magnitude_ratio_brightness))

		obj.used_in_calculations = True

		print(obj.ratio_in_brightness_res)
		
		obj.save()
		
		form = ratio_in_brightness_between_two_stars_Form()
	...
	template_path = "Days/ratio-in-brightness.html"
...
def convert_light_years_in_parsecs(request):

	form = light_years_in_parsecs_Form(request.POST)
	...
	if form.is_valid():
		...
		obj.light_years_in_parsecs = obj.light_years / 3.26

		obj.used_in_calculations = True

		print(obj.light_years_in_parsecs)
		
		obj.save()
		
		form = light_years_in_parsecs_Form()
	...
	template_path = "Days/light-years-in-parsecs.html"
...
def calculate_the_annual_paralax(request):

	form = calculate_the_annual_paralax_Form(request.POST)
	...
	if form.is_valid():
		...
		obj.paralax_res = 1 / obj.distance_from_Earth_paralax

		obj.used_in_calculations = True

		print(obj.paralax_res)
		
		obj.save()
		
		form = calculate_the_annual_paralax_Form()
	...
	template_path = "Days/calculate-the-annual-paralax.html"
...
def calculate_sum_of_attributes_of_nth_day(request):


	if request.method == 'POST':

		form = SumofAttributesofnthDayForm(request.POST)
		...
		if form.is_valid():
			...
			if (obj.nth_day_value_all_attributes > 0 and obj.nth_month_value_all_attributes > 0):


				dl =[]

				dly = [obj.date_to_sum_all_attributes]
				...
				for i in dl:
					...
					dli.append(i.replace(day=obj.nth_day_value_all_attributes, month=obj.nth_month_value_all_attributes))

				for i in dli:

					var = (sum(map(int, str(i.year))) + i.month + i.day + obj.hour_all_attributes+
						obj.minute_all_attributes + obj.second_all_attributes)


				obj.date_to_sum_all_attributes_result = var
				...
				form = SumofAttributesofnthDayForm()
...
def determine_visibility_of_a_star_in_seasons(request):


	if request.method == 'POST':

		form = Determine_the_visibility_of_a_star_in_seasons_Form(request.POST)
		...
		if form.is_valid():
			...
			# 	item.seasons_visibility = " "

			# 	item.save()

			obj.seasons_visibility = " "

			obj.seasons_visibility_var = " "
			...
			for item in qs1:
				...
				if obj.name == item.name:
					...
					obj.ascension = item.ascension

					obj.declination = item.declination
				...
				# print("str")
					...
					print(round(obj.ascension+obj.declination, 2))

					if (round(obj.ascension+obj.declination, 2)  >= 59.06 and  

					round(obj.ascension+obj.declination, 2) <= 89.52):

						print(item.name)

						obj.seasons_visibility += "Jan-Dec, 12 months"

						obj.seasons_visibility_var += "all seasons"
					...
					elif (round(obj.ascension+obj.declination, 2)  >= -9.97  and  

					round(obj.ascension+obj.declination, 2) <= -9.97):
					...
					elif (round(obj.ascension+obj.declination, 2)  >= -46.17  and  

					round(obj.ascension+obj.declination, 2) <= -46.17):
					...
					elif (round(obj.ascension+obj.declination, 2)  >= -46.03  and  

					round(obj.ascension+obj.declination, 2) <= -46.03):
					...
					elif (round(obj.ascension+obj.declination, 2) >= -46.1  and  

					round(obj.ascension+obj.declination, 2) <= -46.1):
					...
					elif (round(obj.ascension+obj.declination, 2)  >= 12.57  and  

					round(obj.ascension+obj.declination, 2) <= 12.57):
					...
					elif (round(obj.ascension+obj.declination, 2)  >= -55.9  and  

					round(obj.ascension+obj.declination, 2) <= -55.9):
					...
					elif (round(obj.ascension+obj.declination, 2)  >= 12.77  and  

					round(obj.ascension+obj.declination, 2) <= 12.77):
					...
					elif (round(obj.ascension+obj.declination, 2)  >= -50.29  and  

					round(obj.ascension+obj.declination, 2) <= -50.29):
					...
					elif (round(obj.ascension+obj.declination, 2)  >= -46.83  and  

					round(obj.ascension+obj.declination, 2) <= -46.83):
					...
					elif (round(obj.ascension+obj.declination, 2)  >= -9.94  and  

					round(obj.ascension+obj.declination, 2) <= -9.94):
					...
					elif (round(obj.ascension+obj.declination, 2)  >= -1.29 and 
					 round(obj.ascension+obj.declination, 2) <= 11.2 ):

						print(item.name)

						obj.seasons_visibility += "Oct-Feb, 5 months"

						obj.seasons_visibility_var += "autumn-winter"
					...
					elif (round(obj.ascension+obj.declination, 2)  >= 20.49 and 

					round(obj.ascension+obj.declination, 2) <= 28.57):

						print(item.name)

						obj.seasons_visibility += "Nov-Apr, 6 months"

						obj.seasons_visibility_var += "autumn-spring"
					...
					elif (round(obj.ascension+obj.declination, 2) >= -6.39 and 

					round(obj.ascension+obj.declination, 2) <=  12.18):

						print("strstrstr")

						print(item.name)

						obj.seasons_visibility += "May-Aug, 4 months"

						obj.seasons_visibility_var += "spring-summer"
					...
					elif (round(obj.ascension+obj.declination, 2)  >= -14.13 
					and round(obj.ascension+obj.declination, 2) <= -14.12):

						print(item.name)

						obj.seasons_visibility += "Feb-May, 4 months"

						obj.seasons_visibility_var += "winter-spring"
					...
					elif (round(obj.ascension+obj.declination, 2)  >= 20.38 and 
					round(obj.ascension+obj.declination, 2) <= 27.45):

						print(item.name)

						obj.seasons_visibility += "Aug-Feb, 7 months"

						obj.seasons_visibility_var += "summer-winter"
					...
					elif (round(obj.ascension+obj.declination, 2) >= 30.89 and 
					round(obj.ascension+obj.declination, 2) <= 41.05):

						print(item.name)

						obj.seasons_visibility += "Aug-Apr, 9 months"
					...
					elif (round(obj.ascension+obj.declination, 2) >= 42 and 
					round(obj.ascension+obj.declination, 2) <= 43):

						print(item.name)

						obj.seasons_visibility += "Jan-Jul, 7 months"

						obj.seasons_visibility_var += "winter-summer"
					...
					elif (round(obj.ascension+obj.declination, 2) >= -4.93 and

						round(obj.ascension+obj.declination, 2) <= -4.92):

						print(item.name)

						obj.seasons_visibility += "Mar-Jul, 5 months"
					...
					elif (round(obj.ascension+obj.declination, 2)  >= 53 and 

					round(obj.ascension+obj.declination, 2) <=  54):

						print(item.name)

						obj.seasons_visibility += "Feb-Jul, 6 months"
					...
					elif (round(obj.ascension+obj.declination, 2) >=-9.2 and 
					round(obj.ascension+obj.declination, 2) <= -1.34):

						print(item.name)

						obj.seasons_visibility += "Apr-Jul, 4 months"
					...
					elif (round(obj.ascension+obj.declination, 2) >= 20.21 and 
					round(obj.ascension+obj.declination, 2) <= 20.22):

						print(item.name)

						obj.seasons_visibility += "June-Oct, 5 months"

						obj.seasons_visibility_var += "summer-autumn"
					...
					elif (round(obj.ascension+obj.declination, 2) >= -8.18 and 
					round(obj.ascension+obj.declination, 2) <= -8.17):

						print(item.name)

						obj.seasons_visibility += "Jan-Apr, 4 months"
					...
					elif (round(obj.ascension+obj.declination, 2) >= 51.52 and

					round(obj.ascension+obj.declination, 2) <= 51.53):

						print(item.name)

						obj.seasons_visibility += "Apr-Nov, 8 months"

						obj.seasons_visibility_var += "spring-autumn"
					...
					elif (round(obj.ascension+obj.declination, 2) >= 42.17 and 
					round(obj.ascension+obj.declination, 2) <= 42.18):

						print(item.name)

						obj.seasons_visibility += "Apr-Oct, 7 months"
					...
					elif (round(obj.ascension+obj.declination, 2) >= 31.71 and 
						round(obj.ascension+obj.declination, 2) <= 65.43):

						print(item.name)

						obj.seasons_visibility += "Jun-Jan, 8 months"
					...
					elif (round(obj.ascension+obj.declination, 2) >= 27.32 and 
					round(obj.ascension+obj.declination, 2) <= 30.46):

						print(item.name)

						obj.seasons_visibility += "Sep-Mar, 7 months"
					...
					elif (round(obj.ascension+obj.declination, 2) >= 60.78 and 
						round(obj.ascension+obj.declination, 2) <= 60.79):

						print(item.name)

						obj.seasons_visibility += "Dec-Aug, 9 months"

						obj.seasons_visibility_var += "winter-autumn"
					...
					elif (round(obj.ascension+obj.declination, 2) >= -14.25 and 
					round(obj.ascension+obj.declination, 2) <= -14.25):

						print(item.name)

						obj.seasons_visibility += "Nov-Feb, 4 months"
			...
			form = Determine_the_visibility_of_a_star_in_seasons_Form()
	...
	template_path = "Days/visibility-of-a-star-in-seasons.html"
...
def calculate_the_flux_density(request):


	form = calculate_the_flux_density_Form(request.POST)
	...
	if form.is_valid():
		...
		obj.flux_density_res =  obj.absolute_magnitude_flux_density / obj.telescope_aperture

		obj.used_in_calculations = True

		print(obj.flux_density_res)
		
		obj.save()
		
		form = calculate_the_flux_density_Form()
	...
	template_path = "Days/calculate-the-flux-density.html"
...
def find_which_stars_are_visible_in_equinox_dates(request):

	if request.method == 'POST':

		form = stars_visible_in_equinox_dates_Form(request.POST)
		...
		if form.is_valid():
			...
			obj1 =  obj.first_id_stars_equinoxes

			obj2 = obj.second_id_stars_equinoxes

			qs = DaysInSearchEngine.objects.filter(id__gte=obj1, id__lte=obj2)

			qs_stars = Stars.objects.all()

			for i in qs_stars:

				i.stars_in_equinox_dates = " "
			...
			for item in qs:
				...
				if (date_var1 == 365 and  item.date_field.month == 3 and item.date_field.day  == 20):
					...
					for i in qs_stars:
						...
						# i.stars_in_equinox_dates += "Spring equinox: "

						i.stars_in_equinox_dates+=item.most_visible_periods_seasons 

						print("str")

						i.save()

				if (date_var1  == 366  and item.date_field.month == 3 and item.date_field.day == 21):
			...
			form = stars_visible_in_equinox_dates_Form()
	...
	if request.user.is_authenticated and request.user.is_staff:
		...
		template_path = "Days/find-which-stars-are-visible-in-equinox-dates.html"
...
def find_which_stars_are_visible_in_autumn_equinox_dates(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			for i in qs_stars:
				...
				i.stars_in_autumn_equinox_dates = " "
			...
			for item in qs:
				...
				if (date_var1 == 365 and  item.date_field.month == 9 and item.date_field.day  == 22):
					...
					for i in qs_stars:
						...
						# i.stars_in_equinox_dates += "Autumn equinox: "

						i.stars_in_autumn_equinox_dates+=item.most_visible_periods_seasons 
				...
				if (date_var1  == 366  and item.date_field.month == 9 and item.date_field.day == 23):
...
def find_which_stars_are_visible_in_summer_solstice_dates(request):

	if request.method == 'POST':

		form = stars_visible_in_solstice_dates_Form(request.POST)
		...
		if form.is_valid():
			...
			obj1 =  obj.first_id_stars_solstices

			obj2 = obj.second_id_stars_solstices
			...
			for item in qs:
				...
				if (date_var1 == 365 and  item.date_field.month == 6 and item.date_field.day  == 20):
					...
					for i in qs_stars:
						...
						i.stars_in_solstice_dates+=item.most_visible_periods_seasons 
				...
				if (date_var1  == 366  and item.date_field.month == 6 and item.date_field.day == 21):
			...
			form = stars_visible_in_solstice_dates_Form()
	...
	if request.user.is_authenticated and request.user.is_staff:
		...
		template_path = "Days/find-which-stars-are-visible-in-solstice-dates.html"
...
def find_which_stars_are_visible_in_winter_solstice_dates(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			for i in qs_stars:
				...
				i.stars_in_winter_solstice_dates = " "
			...
			for item in qs:
				...
				if (date_var1 == 365 and  item.date_field.month == 12 and item.date_field.day  == 21):
					...
					for i in qs_stars:
						...
						i.stars_in_winter_solstice_dates+=item.most_visible_periods_seasons 
				...
				if (date_var1  == 366  and item.date_field.month == 12 and item.date_field.day == 22):
...
def display_stars_most_visible_in_equinox_dates(request):

	if request.method == 'POST':

		form = display_stars_most_visible_in_equinox_dates_Form(request.POST)
		...
		if form.is_valid():
			...
			obj_first = Stars.objects.first()

			obj = Stars.objects.last()

			for i in qs_stars:

				i.stars_in_equinox_dates_res = " "
			...
			var += obj_first.stars_in_equinox_dates.split(",")

			var += obj_first.stars_in_autumn_equinox_dates.split(",")
			...
			for i in qs_stars:
				...
				i.stars_in_equinox_dates_res += ",".join(set(var)).strip(",")

				i.save()

			obj.used_in_calculations = True

			form = display_stars_most_visible_in_equinox_dates_Form()
...
def display_stars_most_visible_in_solstice_dates(request):

	if request.method == 'POST':

		form = display_stars_most_visible_in_solstice_dates_Form(request.POST)
		...
		if form.is_valid():
			...
			var += obj_first.stars_in_solstice_dates.split(",")

			var += obj_first.stars_in_winter_solstice_dates.split(",")
			...
			for i in qs_stars:
				...
				i.stars_in_solstice_dates_res += ",".join(set(var)).strip(",")
			...
			form = display_stars_most_visible_in_solstice_dates_Form()
...
def display_stars_in_equinox_dates(request):

	obj = Stars.objects.first()

	context = {"obj": obj,}

	template_path = "Days/display-stars-in-equinox-dates.html"
...
def display_stars_in_solstice_dates(request):

	obj = Stars.objects.first()

	context = {"obj": obj,}

	template_path = "Days/display-stars-in-solstice-dates.html"
...
def  delete_the_DaysInSearchEngine_using_ids(request):


	if request.method == 'POST':

		form = delete_the_DaysInSearchEngine_using_ids_Form(request.POST)
		...
		if form.is_valid():
			...
			obj_2 = DaysInSearchEngine.objects.filter(id__gte=obj.id_one_objs_to_delete, id__lte=obj.id_two_objs_to_delete)
			...
			form = delete_the_DaysInSearchEngine_using_ids_Form()
...
def set_total_lunar_eclipses(request):
	...
	if request.method == 'POST':
		...
		form = set_total_lunar_eclipses_Form(request.POST)
		...
		if form.is_valid():
			...
			obj1 = obj.first_id_set_total_lunar_eclipses

			obj2 = obj.second_id_set_total_lunar_eclipses
			...
			for item in qs:
				...
				print(item.date_field.year)

				if (sum(map(int, str(item.date_field.year)))== 3 or 
					sum(map(int, str(item.date_field.year)))== 5 or
					sum(map(int, str(item.date_field.year)))== 6 or 
					sum(map(int, str(item.date_field.year))) == 9 or 
					sum(map(int, str(item.date_field.year))) == 1 or 
					sum(map(int, str(item.date_field.year))) == 3 or 
					sum(map(int, str(item.date_field.year))) == 4 or 
					sum(map(int, str(item.date_field.year))) == 7 or 
					sum(map(int, str(item.date_field.year))) == 8 or 
					sum(map(int, str(item.date_field.year))) == 2) :

					item.total_lunar_eclipse = True

					item.save()



			form = set_total_lunar_eclipses_Form()
	...
	if request.user.is_authenticated and request.user.is_staff:
		...
		template_path = "Days/set-total-lunar-eclipses.html"
...
def find_periods_with_total_lunar_eclipses_with_stars_most_visible(request):

	if request.method == 'POST':

		form = lunar_eclipses_stars_most_visible_Form(request.POST)
		...
		if form.is_valid():
			...
			obj1 = obj.first_date_period_total_lunar_eclipse
			obj2 = obj.second_date_period_total_lunar_eclipse



			obj3 = DaysInSearchEngine()

			obj3.qs = DaysInSearchEngine.objects.get_time_frame(obj1, obj2)
			...
			for item in obj3.qs:
				...
				if item.total_lunar_eclipse == True:

					lsmv_str += item.most_visible_periods_seasons
			...
			obj3.var += ",".join(set(lsmv)).strip(",")

			print(obj3.var)

			obj.list_stars_most_visible_lunar_eclipses = obj3.var

			obj.save()


			form = lunar_eclipses_stars_most_visible_Form()
	...
	template_path = "Days/lunar-eclipses-stars-most-visible.html"
...
def determine_the_relative_position_of_stars(request):


	if request.method == 'POST':

		form = determine_the_relative_position_of_stars_Form(request.POST)
		...
		if form.is_valid():
			...
			for item in qs1:
				...
				if (round(item.ascension+item.declination, 2) == 20.6):

					print(item.name)

					item.m24_pleiades_list += str(item.name) + " : " + "7°"

					item.hyades_list += str(item.name) + " : " + "0.2°"

					item.m1_list += str(item.name) + " : " + "7°"

					item.save()



				elif (round(item.ascension+item.declination, 2) == 50.72):

					print(item.name)

					item.m38_list += str(item.name) + " : " + "9°"

					item.m37_list += str(item.name) + " : " + "12°"
				...
				elif (round(item.ascension+item.declination, 2)  == -9.97):

					print(item.name)

					item.m50_list += str(item.name) + " : " + "8°"

					item.m41_list += str(item.name) + " : " + "4°"
				...
				elif (round(item.ascension+item.declination, 2)  == 56.84):

					print(item.name)

					item.m57_list += str(item.name) + " : " + "5°"
				...
				elif (round(item.ascension+item.declination, 2)  == (-3.02)):

					print(item.name)

					item.m42_list += str(item.name) + " : " + "3°"
				...
				elif (round(item.ascension+item.declination, 2) == 27.95):

					print(item.name)

					item.m2_list += str(item.name) + " : " + "7°"
				...
				elif (round(item.ascension+item.declination, 2) == 12.77):

					print(item.name)

					item.m42_list += str(item.name) + " : " + "12°"
				...
				elif (round(item.ascension+item.declination, 2)  == 35.5):

					print(item.name)

					item.m35_list += str(item.name) + " : " + "5°"
				...
				elif (round(item.ascension+item.declination, 2) == 38.91):

					print(item.name)

					item.m35_list += str(item.name) + " : " + "8°"
			...
			form = determine_the_relative_position_of_stars_Form()
	...
	if request.user.is_authenticated and request.user.is_staff:
		...
		template_path = "Days/determine-relative-position-of-stars.html"
...
def display_site_pages(request):

	template_path = "Days/site-pages.html"

	return render(request, template_path)


	
def display_aux_pages(request):

	template_path = "Days/aux-pages.html"
...
def display_astronomy_pages(request):

	template_path = "Days/astronomy.html"
